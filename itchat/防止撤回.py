...
# {msg_id:(msg_from,msg_group,msg_to,msg_time,msg_time_touser,msg_type,msg_content,msg_url)}
msg_dict = {}
...
def SaveMsg(msg):
	...
	if re.search(r"<replacemsg><!\[CDATA\[.*撤回了一条消息]]></replacemsg>", msg['Content']) is not None:
		...
		if old_msg['msg_type'] == "Sharing":
			...
			msg_send += r", 存储在当前目录下files文件夹中"
			shutil.move(old_msg['msg_content'], "files")
